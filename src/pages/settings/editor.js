 ...
 */
export default function editorSettings(actualsettings) {
  // We load the dom
  var contenttext = `
  ...
  function createSetting(key, cat, subcat = false) {
    ...
    contenttext += `
      ...
      subcat ? "/" + subcat : ""
                            ...
                            <span class="setting-title"><b style="opacity: 0.65">Editor:</b>${
                              subcat
  ...
  Object.keys(settingsinfo.editor).forEach((key) => {
    if (JSON.stringify(settingsinfo.editor[key]).startsWith("[")) {
      createSetting(key, "editor");
    } else {
      contenttext += `<div class="setting-group">`;
      Object.keys(settingsinfo.editor[key]).forEach((keyinside) => {
        createSetting(keyinside, "editor", key);
      });
      contenttext += `</div>`;
    }
  });
